...
# --- 1. قاعدة بيانات جديدة لضمان نظافة البيانات ---
conn = sqlite3.connect('moutasem_final_time.db', check_same_thread=False)
c = conn.cursor()
c.execute('CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, task TEXT, time TEXT)')
conn.commit()

# --- 2. إعداد الخلفية (باسم ملفك الصحيح) ---
...
with st.container():
    # مدخل النص
    t_text = st.text_input("ما هي مهمتك الجديدة؟", key="input_task")
    
    # مدخل الوقت - أضفنا له key يجعله ثابتاً ولا يتغير تلقائياً
    t_time = st.time_input("حدد وقت التنفيذ (مثلاً 04:00 PM):", value=datetime.now().time(), key="input_time")
    
    if st.button("حفظ المهمة 🌟"):
        if t_text:
            # هنا يكمن الحل: تحويل الوقت المختار من الصندوق (t_time) إلى نص فوراً
...
if not rows:
    st.info("القائمة فارغة.. أضف مهمة وحدد وقتها!")
else:
    for row in rows:
        col1, col2 = st.columns([5, 1])
        with col1:
            # عرض الوقت الذي تم حفظه في قاعدة البيانات
